 ...
 */
public class RotatingLog extends Logger {
	...
	public static final Character levels[] = {'V', 'D', 'I', 'W', 'E', 'A'};
	 ...
	 * <i>verbose_level</i>
	 */
	public Logger print(String message, int level) { // long
	...
	public Logger println(String tag, String message, int level) {
		long now = Calendar.getInstance().getTime().getTime();
		if (now > next_rotation) {
			rotate();
			updateNextRotationTime();
		...
		String print = String.format("[%s %s/%s] %s", DateUtils.logTimestamp(),
										levels[level-2], tag, message);
		return super.println(print, level);
	}
	...
	private void updateNextRotationTime() { // Calendar
		// cal=GregorianCalendar.getInstance(Locale.CHINA);
