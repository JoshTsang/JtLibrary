package com.joshtttt.photoselector.ui;
...
import com.joshtttt.photoselector.domain.PhotoSelectorDomain;
import com.joshtttt.photoselector.model.AlbumModel;
import com.joshtttt.photoselector.model.PhotoModel;
import com.joshtttt.photoselector.ui.PhotoItem.onItemClickListener;
import com.joshtttt.photoselector.ui.PhotoItem.onPhotoItemCheckedListener;
import com.joshtttt.photoselector.util.AnimationUtil;
import com.joshtttt.photoselector.util.CommonUtils;

public abstract class PhotoSelectorBaseActivity extends Activity implements onItemClickListener, onPhotoItemCheckedListener,
	...
	private static final String TAG = PhotoSelectorBaseActivity.class.getSimpleName();
...
//		CommonUtils.launchActivityForResult(this, new Intent(MediaStore.ACTION_IMAGE_CAPTURE), REQUEST_CAMERA);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == REQUEST_CAMERA && resultCode == RESULT_OK) {
			PhotoModel photoModel = new PhotoModel(mPhotofileName);
			selected.clear();
			File photo = new File(mPhotofileName);
			if (photo.exists()) {
	...
	public void onCheckedChanged(PhotoModel photoModel, CompoundButton buttonView, boolean isChecked) {
		if (isChecked) {
			File photo = new File(photoModel.getOriginalPath());
			if (photo.exists()) {
				selected.add(photoModel);
		...
		if (selected.isEmpty()) {
			...
			setComfirmBtnEnable(true);
		}
	}

	private void setComfirmBtnEnable(boolean b) {
		btnOk.setEnabled(b);
		if (b) {
			btnOk.setTextColor(android.graphics.Color.WHITE);
		} else {
			btnOk.setTextColor(android.graphics.Color.LTGRAY);
		}
	}

	private boolean cancleSeletion() {
		boolean ret = false;
		for (PhotoModel p:selected) {
			p.setChecked(false);
			ret = true;
		}
		selected.clear();
