...
public class Logger {
	...
	public Logger(PrintStream out_stream, String log_tag, int verbose_level) {
	 ...
	 * the MAX_SIZE
	 */
	public Logger(String file_name, String log_tag, int verbose_level) {
		PrintStream os = null;
		if (verbose_level > 0) {
			try {
				os = new PrintStream(new FileOutputStream(file_name, true));
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	/** Logs the Exception */
	public Logger printException(Exception e, int level) { // ByteArrayOutputStream
		// err=new
	...
	public Logger printPacketTimestamp(String proto, String remote_addr,
			int remote_port, int len, String message, int level) {
		String str = remote_addr + ":" + remote_port + "/" + proto + " (" + len
				+ " bytes)";
		if (message != null)
			str += ": " + message;
		println(DateUtils.logTimestamp() + ", " + str, level);
		return this;
	}
